class Ticket:
    ...
    def book(self) -> None:
        if not self.is_booked:
            ...
            print(f"Il biglietto per '{self.movie}' posto è già prenotato.")
...
class Cinema:
    ...
    def cancel_ticket(self, viewer_id: str, ticket_id: str) -> None:
        if viewer_id in self.viewers and ticket_id in self.tickets:
            ...
            print("Spettattore o biglietto non trovato.")
    
    def list_available_tickets(self) -> list[str]:
        return [ticket_id for ticket_id, ticket in self.tickets if not ticket.is_booked]
