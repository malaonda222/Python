...
spaces: dict[int, dict[str, str|None|bool]]
...
class ParkingLot:
    ...
    def remove_space(self, numero: int) -> dict|str:
        ...
        if numero in self.spaces and self.spaces[numero]["occupato"] == True:
            return "Errore: posto occupato."
        if numero in self.spaces and self.spaces[numero]["occupato"] == False:
            rimosso = self.spaces.pop(numero)
            return rimosso
    ...
    def list_free_spaces(self) -> list[int]:
        ...
        for key, value in self.spaces.items():
            if value["occupato"] == False:
                posti_liberi.append(key)
        return posti_liberi
    
        # posti_liberi = []
        # for numero in self.spaces.keys():
        #     if self.spaces[numero]["occupato"] == False:
        #         posti_liberi.append(numero)
        # return posti_liberi

        return [key for key, value in self.spaces.items() if not value["occupato"]]
