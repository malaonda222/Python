'''Progettare un semplice sistema bancario con i seguenti requisiti:
    ...
    Classe Bank:
        Attributi:
            accounts: dict[str, Account] - un dizionario per memorizzare gli account in base 
            ai loro ID.
        Metodi:
            create_account(account_id): crea un nuovo account con l'ID specificato e un saldo
            pari a 0. Solleva il seguente KeyError "Account with this ID already exists" se 
            l'account con l'ID specificato esiste già.
            deposit(account_id, amount): deposita l'importo specificato sul conto con l'ID 
            fornito.
...
class Bank:
    ...
    def deposit(self, account_id: str, amount: float) -> None:
        if account_id not in self.accounts:
            print("Error, account id doesn't exist")
        else:
