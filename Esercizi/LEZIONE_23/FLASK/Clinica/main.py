...
def home():
    return jsonify({
        "message": "Clinic Booking with API",
        "links": {
            "bookings_list": url_for("list_bookings"),
            "booking_sample": url_for("get_booking", booking_id="123"),
            "estime_sample": url_for("get_estimate", booking_id="123", factor=1.0),
            "create_booking": url_for("add_booking"),
            "put_booking": url_for("put_booking", booking_id="123"),
            "patch_booking": url_for("patch_booking", booking_id="123"),
            "delete_booking": url_for("delete_booking")
        }
    }
    )
...
def get_bookings():
    ...
    #oppure 
    # booking_list = []
    # for b in clinic_hub.list_all():
    #   booking_list.append(b)
    # return jsonify({"booking": booking_list})
...
def get_estimate(booking_id, factor):
    booking = clinic_hub.get(booking_id)
    if booking is None:
        return jsonify({"error": "Booking not found"}), 404
...
def add_booking():
    ...
    try:
        booking = _create_booking_from_json(data)
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    
    ok = clinic_hub.add(booking)
    if not ok: 
        return jsonify({"error": "Booking already exist"}), 400
    return jsonify(booking.info()), 201

@app.route('/bookings/<string:booking_id>', methods=['PUT'])
def put_booking(booking_id):
    data = request.get(booking_id)
    if not isinstance(data, dict):
        return jsonify({"error": "Request must be JSON object"}), 400
    data = dict(data)
    ...
    try:
        new_booking = _create_booking_from_json(data)
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    
    clinic_hub.update(booking_id, new_booking)
    return jsonify(new_booking.info())
...
def patch_booking(booking_id):
    ...
    if not isinstance(data, dict) or "status" not in data:
        return jsonify({"error": "Request must be a JSON object with status field"}), 400
    
    new_status = data.get("status")
    if not new_status or not isinstance(new_status, str):
        return jsonify({"error": "Missing or invalid new status"}), 400
...
def delete_booking(booking_id):
    deleted = clinic_hub.delete(booking_id)
    if not deleted:
        return jsonify({"error": "Booking not found"}), 404
