...
def main():
    ...
    #2) GET /animals (lista iniziale fufi, mira)
    r = request.get(f"{BASE_URL}/animals", headers=headers)
    print_response("GET /animals (lista iniziale)", r)

    #3) GET /animals/fufi (dettagli del cane di esempio)
    r = request.get(f"{BASE_URL}/animals/fufi", headers=headers)
    print_response("GET animals/fufi", r)

    #4) GET animals/fufi/food (cibo giornaliero)
    r = request.get(f"{BASE_URL}/animals/fufi/food", headers=headers)
    print_response("GET /animals/fufi/food", r)

    #5) GET animals/fufi/adoption (stato di adozione iniziale, atteso False)
    r = request.json(f"{BASE_URL}/animals/fufi/asoption", headers=headers)
    print_response("GET animals/fufi/adoption (prima dell'adozione)", r)

    #6) POST /animal/add - aggiungiamo un nuovo cane d2
    new_dog_id = "d2"
    new_dog = {
        "type": "dog",
        "id": new_dog_id,
